...
var currentPage = Observable("HometimelineView");
...
function homeClick(){
	srouter.goto("HometimelineView");
}
function notificationClick(){
	srouter.goto("NotificationView");
}
function tagClick(){
	srouter.goto("TagTimelineView");
}
function localtimelineClick(){
	srouter.goto("LocaltimelineView");
}
function localchatClick(){
	srouter.goto("PleromaChatView");
}
function postClick(){
	router.push("PostView");
}
function fedelatetimelineClick(){
	srouter.goto("FedelatetimelineView");
}
function settingClick(){
	router.push("ConfigView");
}
function videoClick(){
...
function drawClick(){
	router.push("SearchResultView");
}

function gachaClick(){
	router.push("MiniGameView");
}
...
function getStreamingdataA(){
	if(FileSystem.existsSync(path)){
		...
		var streamingurl = burl.replace("http","ws") + 'api/v1/streaming/?stream=user&access_token=' + streamconfig.access_token;
		// var streamingurl = "wss://echo.websocket.org/";

		console.log("getStreamingdataA url=" + streamingurl);
		...
		homeconnection.onerror = function (error) {
			if(isConnected){
				...
				console.log(JSON.stringify(homeconnection));
		...
		homeconnection.onmessage = function (e) {
			...
			try{
				...
				if(recvdata.event == "notification"){
					...
					if(localStorage.getItem('LocalNotifyEnabled') != undefined){
...
function getStreamingdataB(){
	if(FileSystem.existsSync(path)){
		...
		var streamingurl = burl.replace("http","ws") + 'api/v1/streaming/?stream=public&access_token=' + streamconfig.access_token;
		// var streamingurl = "wss://echo.websocket.org/";
...
function getStreamingdataC(){
	if(FileSystem.existsSync(path)){
		...
		var streamingurl = burl.replace("http","ws") + 'api/v1/streaming/?stream=hashtag&tag=' + encodeURIComponent(tagname) + '&access_token=' + streamconfig.access_token;
		// var streamingurl = "wss://echo.websocket.org/";
...
function gotoAnotherTagTimelineView(args){
	srouter.goto("AnotherTagTimelineView");
}
function resetSRouter(){
	srouter.getRoute(function(route){
		console.log(JSON.stringify(route));
		if(route[0] == "AnotherTagTimelineView"){
...
function chatWebSocket(){
	if(FileSystem.existsSync(path)){
		...
		if(localStorage.getItem("PleromaChatEnabled") == undefined){
		...
		console.log("chatWebSocket certurl=" + certurl);
...
function doSearch(){
	if(SearchQuery.value != ""){
		...
		api.get_search_result(SearchQuery.value).then(function(res){
			router.push("SearchResultView",{query: SearchQuery.value,result: res})
		});
	}
}
...
function storeNyaan(){
	...
	if(localStorage.getItem("Coin") != undefined){
		_current = Number(localStorage.getItem("Coin"));
	}
	...
	localStorage.setItem("Coin",_current);

	console.log("storeNyaan finished");
}
...
module.exports = {
	...
	gotoAnotherTagTimelineView:gotoAnotherTagTimelineView,
